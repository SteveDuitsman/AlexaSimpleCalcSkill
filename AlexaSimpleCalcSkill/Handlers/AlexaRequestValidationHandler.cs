...
namespace AlexaSimpleCalcSkill.Handlers
{
  public class AlexaRequestValidationHandler : DelegatingHandler
  {
    protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
      ...
      using (var web = new WebClient())
      {
        ...
        if (!(DateTime.TryParse(cert.GetExpirationDateString(), out expirationDate) && expirationDate > DateTime.UtcNow &&
              DateTime.TryParse(cert.GetEffectiveDateString(), out effectiveDate) && effectiveDate > DateTime.UtcNow))
        {
          throw new HttpResponseException(new HttpResponseMessage(HttpStatusCode.BadRequest));
        }
